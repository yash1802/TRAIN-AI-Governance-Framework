"""
=============================================================================
TRAIN Framework — Empirical Validation (Extended: S3 LLM Execution)
...
NOTE: This is the extended version of train_empirical_validation.py.
      It adds live LLM execution for S3 (Offer Generation) via the
      OpenAI API. An OPENAI_API_KEY environment variable is required.

      To run without an API key, use:
          train_empirical_validation.py
...
# ── OpenAI — validate API key before any other imports proceed ────────────────
...
if not _api_key:
    ...
    print()
    print("  This file requires an OpenAI API key to demonstrate S3")
    print("  (Offer Generation LLM) as live executed code.")
    print()
    print("  To set the key:")
    print("    export OPENAI_API_KEY=your_key_here   (macOS/Linux)")
    print("    set OPENAI_API_KEY=your_key_here      (Windows)")
    print()
    print("  To run without an API key, use:")
    print("    train_empirical_validation.py")
    print("=" * 78)
    sys.exit(1)

client = OpenAI(api_key=_api_key)
print("  [S3] OpenAI API key detected — LLM execution enabled.")
...
# =============================================================================
# SECTION 6 — GHOST INVENTORY ANTI-PATTERN CHECK (s7 / Art. 9)
# =============================================================================
...
# =============================================================================
# SECTION 6.5 — S3: OFFER GENERATION LLM  [Art. 50 / r4]
# =============================================================================

def generate_offer_llm(df: pd.DataFrame,
                       rec_importance: list,
                       price_importance: list) -> dict:
    separator("SECTION 6.5 — S3: OFFER GENERATION LLM  [Art. 50 / r4]")

    print("\n  Demonstrating S3 (Offer Generation LLM) as live executed code.")
    print("  Guest record: first Transient guest (consistent with Section 5D)\n")

    # Select guest — consistent with Section 5D single-record walkthrough
    guest_row        = df[df[PROTECTED_ATTRIBUTE] == GROUP_MAJORITY].iloc[0]
    recommended_room = str(guest_row["cf_recommended_room"])
    dynamic_price    = round(float(guest_row["dynamic_price"]), 2)
    top3_factors     = [f for f, _ in rec_importance[:3]]

    guest_context = {
        "recommended_room":   recommended_room,
        "dynamic_price_eur":  dynamic_price,
        "lead_time_days":     int(guest_row["lead_time"]),
        "adults":             int(guest_row["adults"]),
        "stay_length_nights": int(guest_row["stays_in_week_nights"]),
    }

    # ── 6.5A: Prompt construction and LLM call ───────────────────────────────
    subsep("6.5A — Prompt Construction and LLM Call")

    prompt = f"""You are a hotel offer generation assistant.

Generate a concise personalized hotel room upgrade offer.

Constraints:
- Do NOT invent amenities, discounts, or services not provided
- Mention the recommended room type exactly as given
- Mention the exact dynamic price provided (in euros)
- Keep the response under 60 words
- Use a professional, guest-facing tone
- Begin with: "AI-generated offer:"

Guest context:
{guest_context}
"""

    print(f"  Prompt sent to gpt-4.1-mini (temperature=0.3)")
    print(f"  Guest context: {guest_context}\n")

    response = client.chat.completions.create(
        model="gpt-4.1-mini",
        messages=[
            {"role": "system",
             "content": "You generate safe, factual, and compliant hotel offers."},
    ...
    # ── 6.5B: Hallucination guard (r1) ───────────────────────────────────────
    subsep("6.5B — Hallucination Guard  [r1 / Art. 9 Compliance Checkpoint]")

    hallucination_detected = False
    hallucination_reasons  = []

    # Robust numeric price check — within €0.50 tolerance
    numbers_in_offer = re.findall(r'\d+\.?\d*', offer_text)
    price_found = any(
        abs(float(n) - dynamic_price) < 0.50 for n in numbers_in_offer
    )
    if not price_found:
        hallucination_detected = True
        hallucination_reasons.append(
            f"Expected price €{dynamic_price} not found in offer text")

    if recommended_room not in offer_text:
        hallucination_detected = True
        hallucination_reasons.append(
            f"Expected room type '{recommended_room}' not found in offer text")

    forbidden_terms = ["free spa", "airport limousine", "private butler",
                       "complimentary breakfast", "free upgrade"]
    for term in forbidden_terms:
        if term.lower() in offer_text.lower():
            hallucination_detected = True
            hallucination_reasons.append(f"Forbidden term detected: '{term}'")

    if hallucination_detected:
        print(f"  [CONTROL] Hallucination detected:")
        for reason in hallucination_reasons:
            print(f"    - {reason}")
        print(f"  [CONTROL] Non-compliant output replaced with safe fallback.")
        offer_text = "Unable to generate compliant AI offer."
    else:
        print(f"  Hallucination guard          : PASSED ✓")
        print(f"  Price verified               : €{dynamic_price} present ✓")
        print(f"  Room type '{recommended_room}' present : ✓")
        print(f"  No forbidden terms detected  : ✓")

    # ── 6.5C: Transparency card (r4) ─────────────────────────────────────────
    subsep("6.5C — Transparency Card  [r4 / Art. 50 — 'Why this offer?']")

    transparency_card = {
        "AI_Generated_Label" : "AI-generated offer (Art. 50)",
        "Recommended_Room"   : recommended_room,
        "Dynamic_Price_EUR"  : dynamic_price,
        "Top_3_SHAP_Factors" : top3_factors,
        "Explanation"        : (
            f"This offer was generated based on your booking profile. "
            f"The top factors influencing this recommendation are: "
            f"{', '.join(top3_factors)}."
        ),
    }

    print(f"\n  'Why this offer?' transparency card:")
    for k, v in transparency_card.items():
        print(f"    {k:<25} : {v}")

    # ── 6.5D: Output monitoring (r5) ─────────────────────────────────────────
    subsep("6.5D — Output Monitoring  [r5 / Art. 61 Post-Market Monitoring]")

    llm_alerts = []
    if hallucination_detected:
        llm_alerts.append("Hallucination detected; non-compliant output replaced")
    if len(offer_text) > 300:
        llm_alerts.append("Offer text unusually long (>300 chars)")
    if not re.findall(r'\d+\.?\d*', offer_text):
        llm_alerts.append("Price absent from final offer text")

    if llm_alerts:
        print(f"\n  [LLM MONITORING ALERTS]")
        for a in llm_alerts:
            print(f"    - {a}")
    else:
        print(f"\n  LLM output monitoring        : no alerts fired ✓")

    # ── 6.5E: Audit log (r2) ─────────────────────────────────────────────────
    subsep("6.5E — Audit Log  [r2 / Art. 11 Documentation]")

    llm_audit_log = {
        "timestamp"             : datetime.now().isoformat(),
        "model"                 : "gpt-4.1-mini",
        "temperature"           : 0.3,
        "guest_customer_type"   : str(guest_row["customer_type"]),
        "recommended_room"      : recommended_room,
        "dynamic_price_eur"     : dynamic_price,
        "hallucination_detected": hallucination_detected,
        "hallucination_reasons" : hallucination_reasons,
        "alerts_fired"          : len(llm_alerts),
        "final_offer_text"      : offer_text,
    }

    print(f"\n  LLM audit log (r2 — Art. 11 documentation):")
    for k, v in llm_audit_log.items():
        print(f"    {k:<28} : {v}")
...
def build_traceability_matrix(df: pd.DataFrame,
    ...
    llm_results: if provided (LLM version only), S3 cells in Scenario B
                 reflect actual runtime output rather than documented design.
    """
    ...
    if scenario == "A":
        ...
        # ── S3 cells: use live LLM results if available, else documented design
        if llm_results is not None:
            s3_r1 = (
                "Hallucination guard fired; non-compliant output replaced — "
                f"reasons: {'; '.join(llm_results['hallucination_reasons'])}"
                if llm_results["hallucination_detected"]
                else "Hallucination guard executed; offer passed all checks ✓"
            )
            s3_r2 = (
            ...
            s3_r4 = (
                ...
                f"Top-3 SHAP factors: {', '.join(llm_results['top3_factors'])}; "
                f"AI-generated label applied per Art. 50(1)"
            )
            s3_r5 = (
                "LLM alert fired: " + "; ".join(llm_results["llm_alerts"])
                if llm_results["llm_alerts"]
                else "LLM output monitoring active; no alerts fired ✓"
            )
        else:
            # Documented design mode (should not be reached in this file,
            # but kept as a safety fallback)
            s3_r1 = ("Compliance checkpoint + hallucination guard — "
                     "offer fields validated against live PMS inventory "
                     "before delivery; max hallucination rate 0.5%")
            s3_r2 = ("Prompt & output technical documentation — "
                     "system prompt version-controlled; outputs logged")
            s3_r4 = ("'Why this offer?' transparency card — "
                     "AI-generated label per Art. 50(1)")
            s3_r5 = ("Output quality monitoring — anomaly detection on "
...
def print_empirical_summary(df: pd.DataFrame,
    ...
    # LLM-specific summary row
    if llm_results is not None:
        llm_status = (
            "Hallucination detected; output replaced"
            if llm_results["hallucination_detected"]
            else "Guard passed; compliant offer generated ✓"
        )
        llm_row = ("S3 LLM hallucination guard result",
                   llm_status,
                   "Art. 9 / r1 — live execution")
    else:
        llm_row = None
  ...
  ✓ S3 LLM offer generation with live hallucination guard (gpt-4.1-mini)
  ✓ S3 transparency card generated from population-level SHAP factors
  ✓ S3 audit log and output monitoring — Art. 11 and Art. 61 controls
  ...
  ✓ S3 matrix cells reflect actual LLM execution output, not design strings
...
def main():
    print("=" * 78)
    print("  TRAIN FRAMEWORK — EMPIRICAL VALIDATION PIPELINE (LLM EXTENDED)")
  ...
  Purpose : Empirical demonstration addressing Reviewer 1, C1-Step2
            Extended version — S3 executed via OpenAI gpt-4.1-mini
    """)
    ...
    # ── S3: Live LLM execution ────────────────────────────────────────────────
    llm_results = generate_offer_llm(df, rec_importance, price_importance)
